		...
		QUnit.test( "JPL fitted", function( assert )
		{
			for (var i = 0; i < tests.length; i += 1)
			{
				var test = tests[i],
				    ...
				    elp = elpmpp02.jpl.position(jy2k),
				...
				assert.equal(elp.GM, elpmpp02.jpl.GM, "Has Gravitational Parameter");

				// Check after 2 days
				var off = 2 / 365.25;

				// get via theory and via older orbit
				var orbit = elpmpp02.jpl.orbit(jy2k);
				var state1 = elpmpp02.jpl.position(jy2k + off);

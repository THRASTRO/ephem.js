			...
			(function(body)
			{
				...
				QUnit.test( body, function( assert )
				{
					for (var i = 0; i < tests[body].length; i += 1)
					{
						...
						assert.equal(orbit._GM, top2010[body].GM, "Gravitational Parameter");

						// position state vector
						assert.close(
							r_equ.x, test[13], top2010tst.eps * prec.r,
							test[0] + ": Equatorial position x"
						);
						assert.close(
							r_equ.y, test[14], top2010tst.eps * prec.r,
							test[0] + ": Equatorial position y"
						);
						assert.close(
							r_equ.z, test[15], top2010tst.eps * prec.r,
							test[0] + ": Equatorial position z"
						);

						// velocity state vector
						assert.close(
							v_equ.x / 365.25, test[16], top2010tst.eps * prec.r,
							test[0] + ": Equatorial velocity x"
						);
						assert.close(
							v_equ.y / 365.25, test[17], top2010tst.eps * prec.r,
							test[0] + ": Equatorial velocity y"
						);
						assert.close(
							v_equ.z / 365.25, test[18], top2010tst.eps * prec.r,

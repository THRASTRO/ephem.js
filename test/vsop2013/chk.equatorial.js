			...
			(function(body)
			{
				...
				QUnit.test( body, function( assert )
				{
					for (var i = 0; i < tests[body].length; i += 1)
					{
						...
						var orbit = vsop2013[body].orbit(jy2k);
						...
						assert.equal(orbit._GM, vsop2013[body].GM, "Gravitational Parameter");
						...
						assert.close(
							v_equ.x / 365.25, test[16], vsop2013tst.eps * prec.r,
							test[0] + ": Equatorial velocity x"
						);
						assert.close(
							v_equ.y / 365.25, test[17], vsop2013tst.eps * prec.r,
							test[0] + ": Equatorial velocity y"
						);
						assert.close(
							v_equ.z / 365.25, test[18], vsop2013tst.eps * prec.r,

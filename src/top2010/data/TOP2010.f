!
!     -------
!     TOP2010  COMPUTATION OF PLANETARY EPHEMERIDES TOP2010
!     -------
!
! --- Declarations -----------------------------------------------------
!
...
!
      eps   = (23.d0+26.d0/60.d0+21.40960d0/3600.d0)*dgrad
      phi   = -0.05028d0*sdrad
      ceps = cos(eps)
      seps = sin(eps)
      cphi = cos(phi)
...
!
      do ip = 5,9
         ...
         do n = 1,ndat
            t  = t0+(n-1)*step
            tj = t-t2000
            call TOP2010 (tj,ip,fich,nul,el,ierr)
            if (ierr.ne.0) stop1
...
!
!
      subroutine TOP2010 (tdj,ipla,nfile,ifile,r,ierr)
! ----------------------------------------------------------------------
!
...
! --- Output ----------------------------------------------------------- 
!
!     r(6)     Table of elliptic coordinates (real*8).
...
!
      ...
      real*8,        dimension(275738)   :: c,s
      integer,       dimension(275738)   :: m
...
!
      ...
      data freq/
     & 0.5296909690160131d+03,
     & 0.2132990807324141d+03,
     & 0.7478165810226641d+02,
     & 0.3813292747646720d+02,
     & 0.2533634111740826d+02/
!
      data ifile0 /0/
      data nfile0 /'*'/
      save
...
! --- Formats ----------------------------------------------------------
!
1001  format (22x,i1,13x,i1,7x,i2,2x,i6)

################################################################################
# AstroJS VSOP2010 converter script (c) 2016 by Marcel Greter
################################################################################
...
sub gen_vsop2k
{
	...
	my $name = sprintf("vsop2010_%s", $theory);
	my $file = sprintf "data/VSOP2010p%d.dat", $part;
	open(my $fh, "<", $file) or die "could not open $file\n$!";

	# parse vsop2010 data file by lines
	...
	my $code = "// generated by vsop2010.pl\n";
	...
	$code .= "vsop2010.${theory} = VSOP2K('${theory}', $rmu->[$part-1], ${js});\n";
...
if ($target =~ m/^src[\/\\](\d+)-(\w+)/) {
	...
	my $path = sprintf('../../test/vsop2010/%02d-%s.html', $id, $qname);
